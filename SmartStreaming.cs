...
namespace MapEditor
{
	...
	public static class SmartStreaming
	{
		...
		/// <summary>
		/// The model, if the game has it in memory, and null if it does not have it yet — in which case it has
		/// now been asked for and the caller should come back on a later frame.
		///
		/// <see cref="ObjectPreview.LoadObject"/> is the other way of getting one, and it is the wrong one
		/// here: it blocks for as long as it takes, up to two hundred frames, and puts "Loading Model" on the
		...
		/// </summary>
		public static Model RequestModel(int hash)
		{
			...
			// A model the game does not have at all would otherwise be asked for on every frame forever. It is
			// not marked invalid here: it was spawned once already, so whatever is wrong with it now is not
			// something to blacklist the hash over.
